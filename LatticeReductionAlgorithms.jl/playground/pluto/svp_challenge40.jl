...
begin
    function svpchallengedim40seed0()
        str = read(`cat svpchallengedim40seed0.txt`, String)
        # 1) 先頭 [[ と末尾 ]] を削除
        s = replace(str, r"^\s*\[\[|\]\n\]\s*$" => "")
        # 2) ][（または ] [）を行区切り ; に
        s = replace(s, r"\]\s*\[" => ";")
    ...
    function fplll_bkz(β, δ)
        s = read(
            pipeline(`cat svpchallengedim40seed0.txt`, `fplll -a bkz -b $(β) -d $(δ)`),
            String,
        )
        # 1) 文字列中の "\n" を実際の改行に
        s = replace(s, "\\n" => "\n")
    ...
    function fplll_lll(δ)
        s = read(pipeline(`cat svpchallengedim40seed0.txt`, `fplll -a lll -d $(δ)`), String)
        # 1) 文字列中の "\n" を実際の改行に
        s = replace(s, "\\n" => "\n")
